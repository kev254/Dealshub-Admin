...
if (isset($_GET['oid'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM flyers WHERE id = ? AND vendor_id = ?");
    $stmt->bind_param("ii", $flyer_id, $vid);
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['flyer_path']) && count($_FILES['flyer_path']['name']) > 0) {
        ...
        // Clear the existing flyer paths as new ones will be uploaded
        $flyer_paths = [];
...
// Concatenate flyer paths into a comma-separated string
    ...
    if (empty($title) || empty($start_date) || empty($valid_until)) {
        ...
        $stmt = $conn->prepare("UPDATE flyers SET category_id = ?, subcategory_id = ?, title = ?, flyer_path = ?, description = ?, start_date = ?, valid_until = ?, thumbnail_image = ?, is_approved = ? WHERE id = ? AND vendor_id = ?");
        $stmt->bind_param("iissssssiii", $category_id, $subcategory_id, $title, $flyer_pathsStr, $description, $start_date, $valid_until, $thumbnail_image, $is_approved, $flyer_id, $vid);
