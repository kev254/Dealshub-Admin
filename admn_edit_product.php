...
$pvid = isset($_GET['pvid']) ? base64_decode($_GET['pvid']) : null;
$categories = getCategories();
$sub_categories = getSubCategories();
$branches = getBranches($pvid);  // Function to get branches for the vendor
...
if ($pid === null) {
    ...
    header("Location: admn_products");
    exit;
}

// Fetch product details to edit
$products = getProducts($pvid, null, null, $pid);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($category_id) || empty($valid_until) || empty($flyer_paths)) {
        ...
        $stmt->bind_param('iisssdssssssi', $category_id, $subcategory_id, $name, $description, $price, $initial_price, $thumbnail_image, $flyer_pathsStr, $branchesStr, $is_approved, $valid_until, $pid, $pvid);
